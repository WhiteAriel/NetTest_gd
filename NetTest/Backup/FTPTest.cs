...
namespace NetTest
{
    public partial class FTPTest : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private BackgroundWorker m_AsyncWorker2 = new BackgroundWorker();
        private BackgroundWorker m_AsyncWorker3 = new BackgroundWorker();
        //private int iTime = 0;
        ...
        public FTPTest()
        {
            ...
            //m_AsyncWorker.ProgressChanged += new ProgressChangedEventHandler(bwAsync_ProgressChanged);
            m_AsyncWorker2.RunWorkerCompleted += new RunWorkerCompletedEventHandler(bwAsync_RunWorker2Completed);
            m_AsyncWorker2.DoWork += new DoWorkEventHandler(bwAsync2_DoWork);

            m_AsyncWorker3.WorkerSupportsCancellation = true;
            //m_AsyncWorker.ProgressChanged += new ProgressChangedEventHandler(bwAsync_ProgressChanged);
            m_AsyncWorker3.RunWorkerCompleted += new RunWorkerCompletedEventHandler(bwAsync_RunWorker3Completed);
            m_AsyncWorker3.DoWork += new DoWorkEventHandler(bwAsync3_DoWork);
        ...
        private void FTPEnd()
        {
            ...
            //ftpClient.DownFinish = false;
            //MessageBox.Show(LocalDown);
        }
        ...
        private void Login(string address, string username, string password, string port, bool anonymous)
        {
            if (ftpClient == null)
            {
                ftpClient = new FtpClient(address, username, password, 200, Convert.ToInt16(port));
        ...
        private void lvLocalFiles_DragDrop(object sender, DragEventArgs e)
        {
            if ((CurrentEffect == DragDropEffects.Copy) && (((ListView)sender).Name != startingFrom))
            {
                ...
                Thread.Sleep(500);
                this.FTPEnd();
                if (inis.IniReadValue("FTP", "EnableLoop") == "1")
                {
        ...
        {
            this.timerLoop.Enabled = false;
            this.timerLoop.Stop();
            if ((this.RemoteDown != null) && (this.LocalDown != null))
            {
                if(File.Exists(LocalDown)) File.Delete(LocalDown);
                this.ClearDns();
                Thread.Sleep(500);
        ...
        private void FTPDown( )
        {
            ...
            device = devices[iDevice];
            string ip = device.PcapIpAddress;
            strbFile.Append("第"+iDownNum.ToString()+"次测试\r\n");
            strbFile.Append("初始化...\r\n");
            strbFile.Append("网卡: " + device.PcapDescription + "\r\n");
            strbFile.Append("IP地址: " + device.PcapIpAddress + "\r\n");
            strbFile.Append("目的地址: " + inis.IniReadValue("FTP", "Host") + "\r\n");
            ...
            //thread2 = new Thread(DownThread); 
            //thread2.Start();  

            if (iDownNum % 2 == 0)
            {

                if (!m_AsyncWorker2.IsBusy)
                {
            ...
            {
                if (!m_AsyncWorker3.IsBusy)
                {
                    m_AsyncWorker3.RunWorkerAsync();
                }
            }
            ...
            try
            {
                ...
                {
                    //MessageBox.Show(ex.Message);
                    this.lstMessages.Items.Add(ex.Message);
                    ftpClient.Close();
                    ftpClient = null;
                    FtpClient.DownFinish = false;
                    btnFTPStart.Enabled = true;
        ...
        private void timerDown_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                lstMessages.Invalidate();
                lstMessages.Update();
                if (iDownNum < int.Parse(inis.IniReadValue("FTP", "DownNum"))-1)
                {
                    if (File.Exists(LocalDown)) File.Delete(LocalDown);
                    this.ClearDns();
                    Thread.Sleep(3000);
        ...
        private void bwAsync3_DoWork(object sender, DoWorkEventArgs e)
        {
            device.PcapStartCapture();

        }

        private void bwAsync_RunWorker2Completed(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Error != null)
            {
                this.strbFile.Append("FTP错误");
        ...
        private void bwAsync_RunWorker3Completed(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Error != null)
            {
                this.strbFile.Append("FTP错误");
