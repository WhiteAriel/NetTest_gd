...
namespace NetTest
{
    ...
    public partial class WebAnalyze : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public void btnStartAnaly_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //m_AsyncWorkerSave.RunWorkerAsync();
                storeResult();
            }
        ...
        private void rdAnaly(string strfile, ListView lv)
        {
            ...
            {
                AverWebValue.FrameRate = sr.ReadLine();
                AverWebValue.FrameRate += ("\r\n" + sr.ReadLine());
                AverWebValue.FrameRate += ("\r\n" + sr.ReadLine());
            }
        ...
        private bool ShowLVPacketAnalys()
        {
            ...
            //int sumInPage = 0;
            //读取每一行数据，第一次加载数据
            //while (strLine != null && sumInPage < packetPageSize) //文件没有读取到记录或达到一页的展示量2000
            //{
            //    //得到每一单元数据
        ...
        private bool ShowTCPStream()
        {
            ...
            AverWebValue.TcpInfo = "TCP连接信息如下：\t\r\n";
            AverWebValue.TcpInfo += "TCP连接个数 \t" + tcpconnecttimes.ToString() + "\t\r\n";
            AverWebValue.TcpInfo += "TCP上行包个数 \t" + tcpuptimes.ToString() + "\t\r\n";
            AverWebValue.TcpInfo += "TCP上行包流量(字节)  \t" + tcpupflow.ToString() + "\t\r\n";
            AverWebValue.TcpInfo += "TCP下行包个数 \t" + tcpdowntimes.ToString() + "\t\r\n";
            AverWebValue.TcpInfo += "TCP下行包流量(字节)  \t" + tcpdownflow.ToString() + "\t\r\n";
            AverWebValue.TcpInfo += "RTT均值(秒) \t" + averrtt.ToString() + "\t";
            ...
            AverWebValue.TcpEx = "TCP异常信息如下：\t\r\n";
            AverWebValue.TcpEx += "前数据包丢失次数 \t" + tcplastpaclost.ToString() + "\t\r\n";
            AverWebValue.TcpEx += "重复确认次数 \t" + reack.ToString() + "\t\r\n";
            AverWebValue.TcpEx += "乱序次数 \t" + wrongorder.ToString() + "\t\r\n";
            AverWebValue.TcpEx += "重传次数 \t" + redeliver.ToString() + "\t\r\n";
            AverWebValue.TcpEx += "重传率 \t" + redeliverrate.ToString() + "%\t\r\n";
            //删除临时文件
            File.Delete(tmpfileName);
            return true;
        }
        ...
        private bool ShowLVDNSAnalys()
        {
            ...
            while (strLine != null)
            {
                ...
                AverWebValue.AverDNS += yValue;

                ChartDNS.Invoke(addDataDel, ChartDNS, ChartDNS.Series["响应时间(秒)"], xValue++, yValue);

            }
            if (xValue > 0)
                AverWebValue.AverDNS /= (int)xValue;
            AverWebValue.AverDNS = Math.Round(AverWebValue.AverDNS, 6);
            LVDNSAnalys.BeginUpdate();
            //加入最后一批
            for (int i = 0; i < lineCount; i++)
            {
        ...
        private bool ShowLVHTTPAnalys()
        {
            ...
            try
            {
                //if (isSelectPcap)
                //{
                //    retCode = http_anal_tofile1(tmpfileName);      //这个函数应该是读取这一次的数据到临时文件中
                //}
                //else
                //{
                //    retCode = http_anal_tofile2(tmpfileName);       //读取上一次配置的文件
                //}
            ...
            while (strLine != null)
            {
                ...
                AverWebValue.AverHTTP += HttpDelay;
            ...
            //计算服务器响应时间
            if (SumLine > 0)
                AverWebValue.AverHTTP /= SumLine;
            AverWebValue.AverHTTP = Math.Round(AverWebValue.AverHTTP, 6);
    ...
    ref int totalPktCount, int[] rangeCount);
        ...
        private bool ShowLVInOut(string PcapFile, int selectIndex)
        {
            ...
            if (SumLine > 0)
            {
                ...
                AverWebValue.AverInOut /= SumLine;
            }
            AverWebValue.AverInOut = Math.Round(AverWebValue.AverInOut, 2);

            //显示吞吐量最值和均值
            this.InOutMax.Text += MaxInOut.ToString() + "字节";
            this.InOutMin.Text += MinInOut.ToString() + "字节";
            this.InOutAvg.Text += AverWebValue.AverInOut.ToString() + "字节";
        ...
        public bool ShowLVDelayJitter(string PcapFile)
        {
            ...
            if (xValue > 0)
            {
                AverWebValue.AverDelay /= (int)xValue;
                AverWebValue.AverJitter /= (int)xValue;

            }
            AverWebValue.AverDelay = Math.Round(AverWebValue.AverDelay, 6);
            AverWebValue.AverJitter = Math.Round(AverWebValue.AverJitter, 6);

            //显示延时抖动的最值和均值
            this.DelayMax.Text += (MaxDelay * 1000).ToString() + "ms";
            this.DelayMin.Text += (MinDelay * 1000).ToString() + "ms";
            this.DelayAvg.Text += (AverWebValue.AverDelay * 1000).ToString() + "ms";

            this.JitterMax.Text += (MaxJitter * 1000).ToString() + "ms";
            this.JitterMin.Text += (MinJitter * 1000).ToString() + "ms";
            this.JitterAvg.Text += (AverWebValue.AverJitter * 1000).ToString() + "ms";
       ...
       ****************************************************************************************/
        ...
        ****************************************************************************************/
        ...
        private void ResultDisplay2()
        {
            ...
            if (!File.Exists(strTxtResult))
            {
                using (StreamWriter swlog = new StreamWriter(File.Create(strTxtResult), Encoding.Default))
                {
                    ...
                    //保存得到的各种缺陷指标的均值
                    swlog.Write("\r\nWEB测试中各个参数均值如下：\r\n");
                    if (AverWebValue.FrameRate == null)
                        AverWebValue.FrameRate = "WEB分析不成功，无法获得视频帧率";
                    //swlog.Write(AverWebValue.FrameRate + "\t\r\n");
                    swlog.Write("DNS响应平均延时(秒)\t" + AverWebValue.AverDNS.ToString() + "\t\r\n");
                    swlog.Write("HTTP响应平均延时(秒)\t" + AverWebValue.AverHTTP.ToString() + "\t\r\n");
                    swlog.Write("服务器响应平均延时(秒)\t" + AverWebValue.AverHTTP.ToString() + "\t\r\n");
                    swlog.Write("吞吐量均值(字节/秒))\t" + AverWebValue.AverInOut.ToString() + "\t\r\n");
                    swlog.Write("平均延时(秒)\t" + AverWebValue.AverDelay.ToString() + "\t\r\n");
                    swlog.Write("平均抖动(秒)\t" + AverWebValue.AverJitter.ToString() + "\t\r\n");
                    //写TCP连接信息
                    swlog.Write("\r\n" + AverWebValue.TcpInfo + "\r\n");
                    //写入TCP异常信息
                    swlog.Write("\r\n" + AverWebValue.TcpEx + "\r\n");
                    swlog.Close();
                }
        ...
        //    rdAnaly2("data_flow_smoothed.txt");
        //    rdAnaly2("play_flow_smoothed.txt");
        //    rdAnaly2("flv_tag.txt");   
        ...
        //    //确定ChartDataPlayFlow图第一部分的横纵轴
        //    {
        //        maxY = this.ChartDataPlayFlow.Series["累积网络数据流"].Points.FindMaxValue("Y1").YValues[0] > this.ChartDataPlayFlow.Series["累积视频播放流"].Points.FindMaxValue("Y1").YValues[0] ? this.ChartDataPlayFlow.Series["累积网络数据流"].Points.FindMaxValue("Y1").YValues[0] : this.ChartDataPlayFlow.Series["累积视频播放流"].Points.FindMaxValue("Y1").YValues[0];
        //        this.ChartDataPlayFlow.ChartAreas[0].AxisY.Maximum = ((int)(maxY / 100) + 1) * 100;
        //        minY = this.ChartDataPlayFlow.Series["累积网络数据流"].Points.FindMinValue("Y1").YValues[0] > this.ChartDataPlayFlow.Series["累积视频播放流"].Points.FindMinValue("Y1").YValues[0] ? this.ChartDataPlayFlow.Series["累积网络数据流"].Points.FindMinValue("Y1").YValues[0] : this.ChartDataPlayFlow.Series["累积视频播放流"].Points.FindMinValue("Y1").YValues[0];
        //        this.ChartDataPlayFlow.ChartAreas[0].AxisY.Minimum = ((int)(minY / 100) + 1) * 100;
        //        maxX = this.ChartDataPlayFlow.Series["累积网络数据流"].Points.FindMaxValue("X").XValue > this.ChartDataPlayFlow.Series["累积视频播放流"].Points.FindMaxValue("X").XValue ? this.ChartDataPlayFlow.Series["累积网络数据流"].Points.FindMaxValue("X").XValue : this.ChartDataPlayFlow.Series["累积视频播放流"].Points.FindMaxValue("X").XValue;
        //        this.ChartDataPlayFlow.ChartAreas[0].AxisX.Maximum = maxX + 5;
        //        minX = this.ChartDataPlayFlow.Series["累积网络数据流"].Points.FindMinValue("X").XValue > this.ChartDataPlayFlow.Series["累积视频播放流"].Points.FindMinValue("X").XValue ? this.ChartDataPlayFlow.Series["累积网络数据流"].Points.FindMinValue("X").XValue : this.ChartDataPlayFlow.Series["累积视频播放流"].Points.FindMinValue("X").XValue;
        //        this.ChartDataPlayFlow.ChartAreas[0].AxisX.Minimum = minX + 5;
        //    }
        //    //确定ChartDataPlayFlow图第二部分的横纵轴
        //    {
        //        maxY = this.ChartDataPlayFlow.Series["实时网络数据流"].Points.FindMaxValue("Y1").YValues[0] > this.ChartDataPlayFlow.Series["实时视频播放流"].Points.FindMaxValue("Y1").YValues[0] ? this.ChartDataPlayFlow.Series["实时网络数据流"].Points.FindMaxValue("Y1").YValues[0] : this.ChartDataPlayFlow.Series["实时视频播放流"].Points.FindMaxValue("Y1").YValues[0];
        //        this.ChartDataPlayFlow.ChartAreas[1].AxisY.Maximum = ((int)(maxY / 100) + 1) * 100;
        //        minY = this.ChartDataPlayFlow.Series["实时网络数据流"].Points.FindMinValue("Y1").YValues[0] > this.ChartDataPlayFlow.Series["实时视频播放流"].Points.FindMinValue("Y1").YValues[0] ? this.ChartDataPlayFlow.Series["实时网络数据流"].Points.FindMinValue("Y1").YValues[0] : this.ChartDataPlayFlow.Series["实时视频播放流"].Points.FindMinValue("Y1").YValues[0];
        //        this.ChartDataPlayFlow.ChartAreas[1].AxisY.Minimum = ((int)(minY / 100) + 1) * 100;
        //        maxX = this.ChartDataPlayFlow.Series["实时网络数据流"].Points.FindMaxValue("X").XValue > this.ChartDataPlayFlow.Series["实时视频播放流"].Points.FindMaxValue("X").XValue ? this.ChartDataPlayFlow.Series["实时网络数据流"].Points.FindMaxValue("X").XValue : this.ChartDataPlayFlow.Series["实时视频播放流"].Points.FindMaxValue("X").XValue;
        //        this.ChartDataPlayFlow.ChartAreas[1].AxisX.Maximum = maxX + 5;
        //        minX = this.ChartDataPlayFlow.Series["实时网络数据流"].Points.FindMinValue("X").XValue > this.ChartDataPlayFlow.Series["实时视频播放流"].Points.FindMinValue("X").XValue ? this.ChartDataPlayFlow.Series["实时网络数据流"].Points.FindMinValue("X").XValue : this.ChartDataPlayFlow.Series["实时视频播放流"].Points.FindMinValue("X").XValue;
        //        this.ChartDataPlayFlow.ChartAreas[1].AxisX.Minimum = minX + 5;
        //    }

        //    //确定ChartVideoFrameSeq图的横纵轴
        //    maxY = minY = maxX = minX = 0;
        //    foreach (Series iSeries in this.ChartVideoFrameSeq.Series)
        //    {
        //        //必须加上点数判断，不然可能会出现“未将应用的对象赋给---”
        //        if (iSeries.Points.Count == 0)
        //            continue;
        ...
        //    maxX = 5 - maxX % 5 + maxX;
        //    minX = 5 - minX % 5 + minX;
        //    this.ChartVideoFrameSeq.ChartAreas[0].AxisY.Maximum = ((int)(maxY / 100) + 1) * 100;
        //    this.ChartVideoFrameSeq.ChartAreas[0].AxisY.Minimum = ((int)(minY / 100) + 1) * 100;
        //    this.ChartVideoFrameSeq.ChartAreas[0].AxisX.Maximum = maxX;
        //    this.ChartVideoFrameSeq.ChartAreas[0].AxisX.Minimum = minX;
        //    this.ChartVideoFrameSeq.ChartAreas[0].AxisX.Interval = (maxX / 5 > 0) ? (maxX / 5) : 1;

        //    //使能画图区域的滚动条
        //    this.EnableScroll(ChartDataPlayFlow.ChartAreas[0]);
        //    this.EnableScroll(ChartDataPlayFlow.ChartAreas[1]);
        //    this.EnableScroll(ChartVideoFrameSeq.ChartAreas[0]);

        //    //图像重构
        //    this.ChartDataPlayFlow.Invalidate();
        //    this.ChartVideoFrameSeq.Invalidate();
        //    return;

        //}


        //private void rdAnaly2(string strfile)
        //{
        //    if (!File.Exists(strfile))
        //    {
        //        //为了让程序界面更加友好，把弹出的Messagebox去掉了！
        ...
        //        if (strfile.Equals("data_flow_smoothed.txt"))
        //        {
        //            this.ChartDataPlayFlow.Invoke(addDataDel, this.ChartDataPlayFlow, this.ChartDataPlayFlow.Series["累积网络数据流"], Convert.ToDouble(ld[0]), Convert.ToDouble(ld[2]) / 1000);
        //            this.ChartDataPlayFlow.Invoke(addDataDel, this.ChartDataPlayFlow, this.ChartDataPlayFlow.Series["实时网络数据流"], Convert.ToDouble(ld[0]), Convert.ToDouble(ld[1]) / 1000);
        //        }
        //        else if (strfile.Equals("play_flow_smoothed.txt"))
        //        {
        //            this.ChartDataPlayFlow.Invoke(addDataDel, this.ChartDataPlayFlow, this.ChartDataPlayFlow.Series["累积视频播放流"], Convert.ToDouble(ld[0]), Convert.ToDouble(ld[2]) / 1000);
        //            this.ChartDataPlayFlow.Invoke(addDataDel, this.ChartDataPlayFlow, this.ChartDataPlayFlow.Series["实时视频播放流"], Convert.ToDouble(ld[0]), Convert.ToDouble(ld[1]) / 1000);
        //        }
        //        else if (strfile.Equals("flv_tag.txt"))
        //        {
        //            double x = Convert.ToDouble(ld[0]);
        ...
        //            if (ld[3].Equals("I视频帧"))
        //            {
        //                this.ChartVideoFrameSeq.Series[0].Points.AddXY(x, Convert.ToDouble(ld[4]));
        //                this.ChartVideoFrameSeq.Series[2].Points.AddXY(x, Convert.ToDouble(ld[5]));
        //            }
        //            else if (ld[3].Equals("P视频帧"))
        //            {
        //                this.ChartVideoFrameSeq.Series[1].Points.AddXY(x, Convert.ToDouble(ld[4]));
        //                this.ChartVideoFrameSeq.Series[3].Points.AddXY(x, Convert.ToDouble(ld[5]));
        //            }
        //        }
        //    }
        //    sr.Close();
        ...
        /***************************************************************
              对Pcap包分析，得到文件概要、数据包、TCP、DNS、HTTP等信息
          *****************************************************************/
        //private void PcapTcpDnsHttpAnalys2()
        //{
        //    int i = -1;
        //    try
        //    {
        ...
        //    else
        //    {
        //        if (!ShowLVSum2()) WrongReason += "文件概要分析异常 \n";
        //        if (!ShowLVPacketAnalys2()) WrongReason += "数据包解析异常 \n";
        //        if (!ShowTCPStream2()) WrongReason += "TCP流分析异常 \n";
        //        if (!ShowLVDNSAnalys2()) WrongReason += "DNS分析异常 \n";
        //        if (!ShowLVHTTPAnalys2()) WrongReason += "HTTP分析异常 \n";
        //        try
        //        {
        //            DelayJitterAnalys2();
        //        }
        ...
        //向文件概要列表填充数据函数
        //private bool ShowLVSum2()
        //{
        //    //清除原有的项
        //    //LVSum.Items.Clear();

        //    //临时文件名称
        //    string tmpfileName = "dissect.tmp";
        ...
        //向数据包列表填充数据函数
        //private bool ShowLVPacketAnalys2()
        //{
        ...
        //    {
        //        //retCode = pcb_list_tostr(tmpfileName);
        //        retCode = pcb_list_tofile(tmpfileName);
        //    }
        //    catch (System.Exception ex)
        //    {
        //        retCode = -1;
        ...
        //    try
        //    {
        //        //retCode = tcp_stream_tostr(tmpfileName);
        //        retCode = tcp_stream_tofile(tmpfileName);
        //    }
        //    catch (System.Exception ex)
        //    {
        ...
        //    averrtt = Math.Round(averrtt, 3);
        //    AverWebValue.TcpInfo = "TCP连接信息如下：\t\r\n";
        //    //AverWebValue.TcpInfo += "TCP连接个数 \t TCP上行包个数 \t TCP上行包流量 \t TCP下行包个数 \t TCP下行包流量 \t RTT均值\t\r\n";
        //    //AverWebValue.TcpInfo += tcpconnecttimes.ToString() + "\t" + tcpuptimes.ToString() + "\t" + tcpupflow.ToString() + "\t" + tcpdowntimes.ToString() + "\t" + tcpdownflow.ToString() + "\t" + averrtt.ToString();
        ...
        //    try
        //    {
        //        //retCode = tcps_exception_tostr(tmpfileName);
        //        retCode = tcps_exception_tofile(tmpfileName);
        //    }
        //    catch (System.Exception ex)
        //    {
        ...
        //    //创建文件读流
        //    sr = new StreamReader(tmpfileName);
        //    strLine = sr.ReadLine();
        ...
        //        tcpCnt++;
        //        //得到每一单元数据
        //        string[] str = strLine.Split(new Char[] { '\t' }, 10);
        ...
        //    redeliverrate = Math.Round(redeliverrate * 100 /tcpCnt, 3);
        //    AverWebValue.TcpEx = "TCP异常信息如下：\t\r\n";
        //    //AverWebValue.TcpEx += "前数据包丢失次数 \t 重复确认次数 \t 乱序次数 \t 重传次数 \t 重传率 \t\r\n ";
        //    //AverWebValue.TcpEx += tcplastpaclost.ToString() + "\t" + reack.ToString() + "\t" + wrongorder.ToString() + "\t" + redeliver.ToString() + "\t" + redeliverrate.ToString();
        //    AverWebValue.TcpEx += "前数据包丢失次数 \t" + tcplastpaclost.ToString() + "\t\r\n";
        //    AverWebValue.TcpEx += "重复确认次数 \t" + reack.ToString() + "\t\r\n";
        //    AverWebValue.TcpEx += "乱序次数 \t" + wrongorder.ToString() + "\t\r\n";
        //    AverWebValue.TcpEx += "重传次数 \t" + redeliver.ToString() + "\t\r\n";
        ...
        //HTTP信息获取函数
        //[DllImport("NetpryDll.dll")]
        //public extern static int http_anal_tostr(string tmpfileName);
        //public extern static int http_anal_tofile(string tmpfileName);
        //private bool ShowLVHTTPAnalys2()
        //{
        ...
        //    try
        //    {
        //        //retCode = http_anal_tostr(tmpfileName);
        //        retCode = http_anal_tofile(tmpfileName);
        //    }
        //    catch (System.Exception ex)
        //    {
        //        retCode = -1;
        ...
        /********************************************************************************
                             对Pcap包分析，得到吞吐量和帧长列表           
         ********************************************************************************/
        //private void InOutFrameLenAnalys2()
        //{

        //    //前期分析方案,可能会因为有一个解析不成功(非错误)而导致整个解析过程失败
        //    //bool AnalysOK = (ShowLVInOut(PcapFileName, ScaleComboBox.SelectedIndex)) && (ShowLVFrameLength());
        //    //return AnalysOK;

        //    if (!ShowLVInOut2(PcapFileName, ScaleComboBox.SelectedIndex))
        //        WrongReason += "吞吐量分析异常 \n";
        //    if (!ShowLVFrameLength2())
        //        WrongReason += "帧长分布分析异常 \n";
        //    return;
        ...
        //ref int totalPktCount, int[] rangeCount);
        //向吞吐量列表添加数据
        //private bool ShowLVInOut2(string PcapFile, int selectIndex)
        //{
        //    //PcapFileName = inis.IniReadValue("Flv", "PcapFile");
        //    //TxtFileName = inis.IniReadValue("Flv", "PlayerFile");
        //    int index = selectIndex;
        ...
        //向帧长分析列表添加数据
        //private bool ShowLVFrameLength2()
        //{
        //    //清空帧长分布列表
        //   // LVFrameLength.Items.Clear();
        ...
        //private void DelayJitterAnalys2()
        //{


        //    //前期分析方案,可能会因为有一个解析不成功(非错误)而导致整个解析过程失败
        //    //bool AnalysOK = ShowLVDelayJitter(PcapFileName);
        //    //return AnalysOK;
        //    bool AnalysOK = false;
        //    //加try catch可以避免延时抖动分析代码中bug对整个分析结果的影响
        //    try
        //    {
        //        AnalysOK = ShowLVDelayJitter2(PcapFileName);
        //        if (!AnalysOK)
        //            WrongReason += "延时抖动分析异常 \n";
        //    }
        //    catch (System.Exception ex)
        ...
        //向延时抖动列表添加数据
        //[DllImport("DelayJitter.dll")]
        //public extern static int DelayJitterAnalyze(string pcapfilename);
        //public bool ShowLVDelayJitter2(string PcapFile)
        //{
        //    string tmpfileName = "DelayJitter.txt";
        ...
        //        //滤除掉那些由于操作导致的过大的延时和抖动
        //        if (float.Parse(str[2]) > 2.0 || float.Parse(str[3]) > 2.0)
        //        {
        //            strLine = sr.ReadLine();
        //            continue;
        //        }
        ...
        //批量处理
        //private void numWorkProcess()
        //{
        //    WaitWindowClass numerousWork = new WaitWindowClass();
        //    numerousWork.Show("批量处理抓包文件中，请等待...");
        ...
        //        InitChart();
        //        //抓包文件名
        //        string capFileName = datalist[index].ToString();
        //        //inis.IniWriteValue("Flv", "PcapFile", capFileName);

        //        //测试报告excel形式
        //        string shortName = (capFileName.Remove(capFileName.Length - 4) + ".xlsx");
        //        //inis.IniWriteValue("Flv", "LogFile", shortName);

        //        //播放过程中视频参数信息txt
        //        shortName = (capFileName.Remove(capFileName.Length - 4) + ".txt");
        //        //inis.IniWriteValue("Flv", "PlayerFile", shortName);

        //        //测试过程信息
        //        shortName = (capFileName.Remove(capFileName.Length - 4) + "-log.txt");
        //        //inis.IniWriteValue("Flv", "LogResult", shortName);

        //        //PcapFileName = inis.IniReadValue("Flv", "PcapFile");
        //        //TxtFileName = inis.IniReadValue("Flv", "PlayerFile");
        //        try
        //        {
        //            //各个部分的解析函数，里面只包含解析之后的内容，而不包含将内容写到listview中

        //            WebInfoAnalys2();         //web播放的相关内容解析
        //            PcapTcpDnsHttpAnalys2();  //分析文件概要、数据包、TCP、DNS、HTTP等信息
        //            InOutFrameLenAnalys2();    //分析吞吐量、帧长分布信息
        //           // DelayJitterAnalys2();      //分析延时抖动信息
        //            ResultDisplay2();          //测试报告结果显示
        //            storeResult();            //保存测试报告
        ...
        //    numerousWork.Dispose();
        //    MessageBox.Show("已处理完成所有的抓包文件");
        //    if (File.Exists("allpcapfile.txt")) 
        //        File.Delete("allpcapfile.txt");//保存所有的cap包的名字
